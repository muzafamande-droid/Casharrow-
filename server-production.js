...
app.get("/member.html", (req, res) => {
  try {
    ...
    const scripts = '<script src="/rental-catalog.js?v=photos6"></script><script src="/aveilot-machine-catalog-fix.js?v=2"></script><script src="/aveilot-real-machine-photos.js?v=1"></script>';
...
app.get("/", (req, res) => {
  ...
  let html = fs.readFileSync(file, "utf8");
  const scripts = '<script src="/member-dashboard-v2.js?v=14"></script><script src="/member-dashboard-boot.js?v=3"></script><script src="/casharrow-ui-fixes.js?v=5"></script><script src="/rental-catalog.js?v=photos6"></script><script src="/aveilot-machine-catalog-fix.js?v=2"></script><script src="/aveilot-real-machine-photos.js?v=1"></script>';
  html = html.replace("</body>", `${scripts}</body>`);
  res.set("Cache-Control", "no-store, no-cache, must-revalidate, proxy-revalidate");
  res.set("Pragma", "no-cache");
  res.set("Expires", "0");
