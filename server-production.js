...
const paymentSettings = require("./payment-settings-routes");
...
app.disable("x-powered-by");app.use(express.json({limit:"5mb"}));app.use(express.urlencoded({extended:true,limit:"5mb"}));
...
app.get("/member.html",(req,res)=>{try{const file=path.join(__dirname,"public","member.html");let html=fs.readFileSync(file,"utf8");html=html.replaceAll("CashArrow","AVEILOT");const scripts='<link rel="stylesheet" href="/aveilot-deep-blue-ui.css?v=2"><script src="/rental-catalog.js?v=custom-machines-3"></script><script src="/aveilot-machine-details-fix.js?v=4"></script><script src="/machine-photo-display.js?v=3"></script><script src="/aveilot-machine-catalog-fix.js?v=5"></script><script src="/aveilot-withdrawal-fee.js?v=2"></script><script src="/aveilot-member-polish.js?v=1"></script><script src="/aveilot-rental-earnings-ui.js?v=1"></script><script src="/aveilot-customer-care.js?v=1"></script><script src="/aveilot-login-notifications.js?v=1"></script><script src="/aveilot-referral-cinematic.js?v=1"></script><script src="/aveilot-referral-dashboard.js?v=1"></script>';html=html.replace("</body>",`${scripts}${AVEILOT_BRANDING_SCRIPT}</body>`);res.set("Cache-Control","no-store, no-cache, must-revalidate, proxy-revalidate");res.set("Pragma","no-cache");res.set("Expires","0");res.type("html").send(html)}catch(error){console.error("AVEILOT member dashboard failed to load:",error);res.status(500).send("Unable to load member dashboard")}});
app.get("/admin.html",(req,res)=>{try{const file=path.join(__dirname,"public","admin.html");let html=fs.readFileSync(file,"utf8");const scripts='<script src="/admin-machine-manager-v2.js?v=3"></script><script src="/admin-payment-settings.js?v=1"></script><script src="/admin-financial-polish.js?v=2"></script><script src="/admin-nav.js?v=1"></script><script src="/aveilot-admin-support.js?v=1"></script>';html=html.replace("</body>",`${scripts}${AVEILOT_BRANDING_SCRIPT}</body>`);res.set("Cache-Control","no-store, no-cache, must-revalidate, proxy-revalidate");res.set("Pragma","no-cache");res.set("Expires","0");res.type("html").send(html)}catch(error){console.error("AVEILOT admin panel failed to load:",error);res.status(500).send("Unable to load admin panel")}});
...
app.post("/api/rentals/:id/complete",(req,res)=>res.status(410).json({success:false,message:"Manual rental completion is disabled. Rental income is credited automatically each day."}));
app.use("/api",accountPg.router);app.use("/api",memberPg.router);app.use("/api",pgFinancial.router);app.use("/api",paymentSettings);app.use("/api",rental.router);app.use("/api",mobileMoney.router);app.use("/api",adminProducts.router);app.use("/api",support.router);
app.get("/",(req,res)=>{const file=path.join(__dirname,"public","index.html");let html=fs.readFileSync(file,"utf8");const scripts='<script src="/guest-home-polish.js?v=2"></script><script src="/aveilot-auth-polish.js?v=2"></script><script src="/aveilot-motion.js?v=2"></script>';html=html.replace("</body>",`${scripts}${AVEILOT_BRANDING_SCRIPT}</body>`);res.set("Cache-Control","no-store, no-cache, must-revalidate, proxy-revalidate");res.set("Pragma","no-cache");res.set("Expires","0");res.type("html").send(html)});
